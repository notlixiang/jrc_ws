...
std::vector<double> observe_up_right={-0.48886, -2.0710, -0.69474, 1.80261, -1.941925, -1.98175};
std::vector<double> observe_down_right={-0.5247, -2.215, -1.4257, 2.5922, -1.9613, -2.0433};
std::vector<double> observe_up_left={0.753727, -1.90678, -0.94593, 1.84789, -1.2250, -0.9789};
std::vector<double> observe_down_left={0.712316, -2.022532, -1.73385, 2.76506, -1.2495, -0.98045};
std::vector<double> observe_up_middle={0.202698, -1.71248, -1.09599, 1.8336, -1.44220, -1.41040};
std::vector<double> observe_down_middle={0.07430, -2.05694, -1.6921, 2.69200, -1.61362, -1.51945};
...
std::vector<double> observe_desk_left={0.7587, -1.800, -1.0839, 1.7442, -1.2261, -0.9725};
std::vector<double> observe_desk_right={-0.30423, -1.84358, -1.0144, 1.7967, -1.746, -1.914};
...
std::vector<double> pre_desk_right = {-0.3461, -1.6621, -1.9078, 3.6097, -1.945, -1.552};
std::vector<double> pre_desk_left = {0.8099, -1.607, -1.96787, 3.6374, -0.790, -1.609};
...
double desk_z_down = 0.16;
double desk_x_up = 0.5;
double desk_y_down = -1.5;
double desk_z_up = 0.40;
...
enum M_robot_state{s_home,s_place,s_grasp_up,s_grasp_down,s_grasp_desk,
...
bool pose_plan(std::string planning_group, std::string reframe, geometry_msgs::Pose target_pose, int pos)
{
    ...
    if(pos == 0)
        ...
        move_group.setPlannerId("DESKRPRMkConfigDefault");
    else if(pos == 8)
        move_group.setPlannerId("DESKMPRMkConfigDefault");
    else if(pos == 9)
        move_group.setPlannerId("DESKLPRMkConfigDefault");
    else
        move_group.setPlannerId("RRTConnectkConfigDefault");
...
bool Joint_plan(std::string planning_group, std::string reframe, std::vector<double> joint_group_positions,int pos)
{
    ...
    {
        ROS_WARN("Joint planning fail, try again!");
        ...
        {
            ROS_WARN("Joint planning fail again, try RRT_connect!");
            move_group.setPlannerId("RRTConnectkConfigDefault");
            success = (move_group.plan(my_plan) == moveit::planning_interface::MoveItErrorCode::SUCCESS);
            if(success)
            {
            ...
            || m_robot_state == s_down_left_observe || m_robot_state == s_down_right_observe || m_robot_state == s_down_middle_observe)
    {
        if(target_pose.position.z>0)
        {
            if(target_pose.position.x>=-0.2 && target_pose.position.x<=0.2)
            {
                ...
                {
                    std::cout<<std::endl;
                    std::cout<<"trac_ik failed , use pose planning!"<<std::endl;
                    std::cout<<std::endl;
                    bool execute_result = pose_plan(planning_group,"base",target_pose,3);
                    if(execute_result)
                        return 1;
                    else
                        return 0;
            ...
            else if(target_pose.position.x<-0.2)
            {
                ...
                else
                {
                    ...
                    std::cout<<"trac_ik failed , use pose planning!"<<std::endl;
                    std::cout<<std::endl;
                    bool execute_result = pose_plan(planning_group,"base",target_pose,1);
                    if(execute_result)
                        return 1;
                    else
                        return 0;
            ...
            else if(target_pose.position.x>0.2)
            {
                ...
                else
                {
                    ...
                    std::cout<<"trac_ik failed , use pose planning!"<<std::endl;
                    std::cout<<std::endl;
                    bool execute_result = pose_plan(planning_group,"base",target_pose,5);
                    if(execute_result)
                        return 1;
                    else
                        return 0;
        ...
        else
        {
            if(target_pose.position.x>=-0.2 && target_pose.position.x<=0.2)
            {
                ...
                else
                {
                    ...
                    std::cout<<"trac_ik failed , use pose planning!"<<std::endl;
                    std::cout<<std::endl;
                    bool execute_result = pose_plan(planning_group,"base",target_pose,4);
                    if(execute_result)
                        return 1;
                    else
                        return 0;
            ...
            else if(target_pose.position.x<-0.2)
            {
                ...
                else
                {
                    ...
                    std::cout<<"trac_ik failed , use pose planning!"<<std::endl;
                    std::cout<<std::endl;
                    bool execute_result = pose_plan(planning_group,"base",target_pose,2);
                    if(execute_result)
                        return 1;
                    else
                        return 0;
            ...
            else if(target_pose.position.x>0.2)
            {
                ...
                else
                {
                    ...
                    std::cout<<"trac_ik failed , use pose planning!"<<std::endl;
                    std::cout<<std::endl;
                    bool execute_result = pose_plan(planning_group,"base",target_pose,6);
                    if(execute_result)
                        return 1;
                    else
                        return 0;
        ...
        if(target_pose.position.x>=-0.2 && target_pose.position.x<=0.2)
        {
            ...
            if(trac_ik_solver(pre_desk_middle,target_pose,result))
            {
                for(int i=0;i<6;i++)
                    std::cout<<result[i]<<"   ";
                std::cout<<std::endl;
                bool execute_result = Joint_plan(planning_group,"base",result,8);
                if(execute_result)
                    return 1;
                else
                    return 0;
            ...
            else
            {
                ...
                std::cout<<"trac_ik failed , use pose planning!"<<std::endl;
                std::cout<<std::endl;
                bool execute_result = pose_plan(planning_group,"base",target_pose,8);
                if(execute_result)
                    return 1;
                else
                    return 0;
        ...
        else if(target_pose.position.x<-0.2)
        {
            ...
            if(trac_ik_solver(pre_desk_right,target_pose,result))
            {
                for(int i=0;i<6;i++)
                    std::cout<<result[i]<<"   ";
                std::cout<<std::endl;
                bool execute_result = Joint_plan(planning_group,"base",result,7);
                if(execute_result)
                    return 1;
                else
                    return 0;
            ...
            else
            {
                ...
                std::cout<<"trac_ik failed , use pose planning!"<<std::endl;
                std::cout<<std::endl;
                bool execute_result = pose_plan(planning_group,"base",target_pose,7);
                if(execute_result)
                    return 1;
                else
                    return 0;
        ...
        else if(target_pose.position.x>0.2)
        {
            ...
            if(trac_ik_solver(pre_desk_left,target_pose,result))
            {
                for(int i=0;i<6;i++)
                    std::cout<<result[i]<<"   ";
                std::cout<<std::endl;
                bool execute_result = Joint_plan(planning_group,"base",result,9);
                if(execute_result)
                    return 1;
                else
                    return 0;
            ...
            else
            {
                ...
                std::cout<<"trac_ik failed , use pose planning!"<<std::endl;
                std::cout<<std::endl;
                bool execute_result = pose_plan(planning_group,"base",target_pose,9);
                if(execute_result)
                    return 1;
                else
                    return 0;
...
bool C_plan(std::string planning_group, std::string reframe, std::vector<geometry_msgs::Pose> waypoints)
{
    ...
    {
        ROS_WARN("C Planning fail, try again");
        sleep(1);
...
bool move_to_observe_desk_left()
{
    ...
    bool execute_result;
    if(m_robot_state==s_place)
        execute_result = Joint_plan(PLANNING_GROUP,"base",observe_desk_left,9);
    else
        execute_result = Joint_plan(PLANNING_GROUP,"base",observe_desk_left,0);
    if(execute_result)
    {
...
bool move_to_observe_desk_right()
{
    ...
    bool execute_result;
    if(m_robot_state==s_place)
        execute_result = Joint_plan(PLANNING_GROUP,"base",observe_desk_right,7);
    else
        execute_result = Joint_plan(PLANNING_GROUP,"base",observe_desk_right,0);
    if(execute_result)
    {
...
{
    ROS_INFO("move_to_desk_middle_observe");
    std::string PLANNING_GROUP = "arm";
    bool execute_result;
    if(m_robot_state==s_place)
        execute_result = Joint_plan(PLANNING_GROUP,"base",observe_desk_middle,8);
    else
        execute_result = Joint_plan(PLANNING_GROUP,"base",observe_desk_middle,0);
    if(execute_result)
    {
        ROS_INFO("move to observe desk_middle succeed");
        m_robot_state = s_observe_desk_middle;
        return true;
    }
    else
    {
        ROS_ERROR("move to observe desk_middle fail");
        return false;
    }
}
...
int getCurrentRegion_desk(geometry_msgs::Pose pose_)
{
    if(pose_.position.x>=-0.2 && pose_.position.x<=0.2)
    {
        return 8;
