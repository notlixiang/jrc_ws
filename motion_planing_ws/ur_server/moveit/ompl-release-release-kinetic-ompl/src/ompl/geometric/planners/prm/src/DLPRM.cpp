...
std::vector<int> path_boost_DL;
std::vector<int> path_index_DL;

ompl::geometric::DLPRM::DLPRM(const base::SpaceInformationPtr &si) : base::Planner(si, "DLPRM")
{
//    specs_.approximateSolutions = true;
//    specs_.directed = true;

    std::cout<<"DLPRM"<<std::endl<<std::endl;

    std::ifstream ist("/home/smj/ros/jrc3/moveit_ws/graph_down_left.xml");
...
double ompl::geometric::DLPRM::m_distance(std::vector<double> q1, std::vector<double> q2)
{
    double distance_=0;
...
void ompl::geometric::DLPRM::adjust_graph()
{
...
ompl::geometric::DLPRM::~DLPRM()
{}

void ompl::geometric::DLPRM::clear()
{
    Planner::clear();
}

void ompl::geometric::DLPRM::setup()
{
    Planner::setup();
}

ompl::base::PlannerStatus ompl::geometric::DLPRM::solve(const base::PlannerTerminationCondition &ptc)
{
    checkValidity();
    adjust_graph();
    ...
    path_boost_DL.clear();
    for (; t != ipot_st; t = p[t])
        path_boost_DL.push_back(t);
    path_boost_DL.push_back(ipot_st);
    std::reverse(path_boost_DL.begin(), path_boost_DL.end());

    //--------------------------------------------------------
//    for(unsigned int i=0;i<path_boost_DL.size();i++)
//    {
//        std::cout<<path_boost_DL[i]<<"   ";
//    }
//    std::cout<<std::endl<<std::endl;
    ...
    for (unsigned int i = 0 ; i < path_boost_DL.size() ; i++)
    {
        ...
        for (unsigned int j = 0 ; j < si_->getStateDimension() ; ++j)
        {
            rstate->values[j] = vectormap[path_boost_DL[i]][j];
        }
        path->append(rstate);
    }
    ...
    //--------------------------------------------------------
//    boost::property_map<Graph_,vertex_state_str>::type stringmap=get(vertex_state_str(),g);
//    for(unsigned int m=0;m<path_index_DL.size();m++)
//    {
//        std::cout<<"path id "<<path_index_DL[m];
//        std::cout<<stringmap[path_index_DL[m]]<<std::endl;
//    }
...
void ompl::geometric::DLPRM::optimizationPath(PathGeometric *path_old,PathGeometric *path_new)
{
    ...
    //--------------------------------------------------------
    path_index_DL.clear();
    path_index_DL.push_back(path_boost_DL[0]);
    //--------------------------------------------------------
    ...
    while(index != (old_path_states.size()-1))
    {
        ...
        for(i=(old_path_states.size()-1);i>(index+1);i--)
        {
            if(si_->checkMotion(path_new->getState(path_new->getStateCount()-1),old_path_states[i]))
            {
                ...
                //--------------------------------------------------------
                path_index_DL.push_back(path_boost_DL[i]);
                //--------------------------------------------------------
                index = i;
                break;
            }
