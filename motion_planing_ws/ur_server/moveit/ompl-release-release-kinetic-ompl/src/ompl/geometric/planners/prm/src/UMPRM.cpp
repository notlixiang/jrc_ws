...
std::vector<int> path_boost_UM;
std::vector<int> path_index_UM;

ompl::geometric::UMPRM::UMPRM(const base::SpaceInformationPtr &si) : base::Planner(si, "UMPRM")
{
//    specs_.approximateSolutions = true;
//    specs_.directed = true;

    std::cout<<"UMPRM"<<std::endl<<std::endl;

    std::ifstream ist("/home/smj/ros/jrc3/moveit_ws/graph_up_middle.xml");
...
double ompl::geometric::UMPRM::m_distance(std::vector<double> q1, std::vector<double> q2)
{
    double distance_=0;
...
void ompl::geometric::UMPRM::adjust_graph()
{
...
ompl::geometric::UMPRM::~UMPRM()
{}

void ompl::geometric::UMPRM::clear()
{
    Planner::clear();
}

void ompl::geometric::UMPRM::setup()
{
    Planner::setup();
}

ompl::base::PlannerStatus ompl::geometric::UMPRM::solve(const base::PlannerTerminationCondition &ptc)
{
    checkValidity();
    adjust_graph();
    ...
    path_boost_UM.clear();
    for (; t != ipot_st; t = p[t])
        path_boost_UM.push_back(t);
    path_boost_UM.push_back(ipot_st);
    std::reverse(path_boost_UM.begin(), path_boost_UM.end());

    //--------------------------------------------------------
//    for(unsigned int i=0;i<path_boost_UM.size();i++)
//    {
//        std::cout<<path_boost_UM[i]<<"   ";
//    }
//    std::cout<<std::endl<<std::endl;
    ...
    for (unsigned int i = 0 ; i < path_boost_UM.size() ; i++)
    {
        ...
        for (unsigned int j = 0 ; j < si_->getStateDimension() ; ++j)
        {
            rstate->values[j] = vectormap[path_boost_UM[i]][j];
        }
        path->append(rstate);
    }
    ...
    //--------------------------------------------------------
//    boost::property_map<Graph_,vertex_state_str>::type stringmap=get(vertex_state_str(),g);
//    for(unsigned int m=0;m<path_index_UM.size();m++)
//    {
//        std::cout<<"path id "<<path_index_UM[m];
//        std::cout<<stringmap[path_index_UM[m]]<<std::endl;
//    }
...
void ompl::geometric::UMPRM::optimizationPath(PathGeometric *path_old,PathGeometric *path_new)
{
    ...
    //--------------------------------------------------------
    path_index_UM.clear();
    path_index_UM.push_back(path_boost_UM[0]);
    //--------------------------------------------------------
    ...
    while(index != (old_path_states.size()-1))
    {
        ...
        for(i=(old_path_states.size()-1);i>(index+1);i--)
        {
            if(si_->checkMotion(path_new->getState(path_new->getStateCount()-1),old_path_states[i]))
            {
                ...
                //--------------------------------------------------------
                path_index_UM.push_back(path_boost_UM[i]);
                //--------------------------------------------------------
                index = i;
                break;
            }
