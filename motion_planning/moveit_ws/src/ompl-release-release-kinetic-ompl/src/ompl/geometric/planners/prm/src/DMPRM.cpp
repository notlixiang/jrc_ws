...
std::vector<int> path_boost_DM;
std::vector<int> path_index_DM;

ompl::geometric::DMPRM::DMPRM(const base::SpaceInformationPtr &si) : base::Planner(si, "DMPRM")
{
//    specs_.approximateSolutions = true;
//    specs_.directed = true;

    std::cout<<"DMPRM"<<std::endl<<std::endl;

    std::ifstream ist("/home/smj/ros/jrc3/moveit_ws/graph_down_middle.xml");
...
double ompl::geometric::DMPRM::m_distance(std::vector<double> q1, std::vector<double> q2)
{
    double distance_=0;
...
void ompl::geometric::DMPRM::adjust_graph()
{
...
ompl::geometric::DMPRM::~DMPRM()
{}

void ompl::geometric::DMPRM::clear()
{
    Planner::clear();
}

void ompl::geometric::DMPRM::setup()
{
    Planner::setup();
}

ompl::base::PlannerStatus ompl::geometric::DMPRM::solve(const base::PlannerTerminationCondition &ptc)
{
    checkValidity();
    adjust_graph();
    ...
    path_boost_DM.clear();
    for (; t != ipot_st; t = p[t])
        path_boost_DM.push_back(t);
    path_boost_DM.push_back(ipot_st);
    std::reverse(path_boost_DM.begin(), path_boost_DM.end());

    //--------------------------------------------------------
//    for(unsigned int i=0;i<path_boost_DM.size();i++)
//    {
//        std::cout<<path_boost_DM[i]<<"   ";
//    }
//    std::cout<<std::endl<<std::endl;
    ...
    for (unsigned int i = 0 ; i < path_boost_DM.size() ; i++)
    {
        ...
        for (unsigned int j = 0 ; j < si_->getStateDimension() ; ++j)
        {
            rstate->values[j] = vectormap[path_boost_DM[i]][j];
        }
        path->append(rstate);
    }
    ...
    //--------------------------------------------------------
//    boost::property_map<Graph_,vertex_state_str>::type stringmap=get(vertex_state_str(),g);
//    for(unsigned int m=0;m<path_index_DM.size();m++)
//    {
//        std::cout<<"path id "<<path_index_DM[m];
//        std::cout<<stringmap[path_index_DM[m]]<<std::endl;
//    }
...
void ompl::geometric::DMPRM::optimizationPath(PathGeometric *path_old,PathGeometric *path_new)
{
    ...
    //--------------------------------------------------------
    path_index_DM.clear();
    path_index_DM.push_back(path_boost_DM[0]);
    //--------------------------------------------------------
    ...
    while(index != (old_path_states.size()-1))
    {
        ...
        for(i=(old_path_states.size()-1);i>(index+1);i--)
        {
            if(si_->checkMotion(path_new->getState(path_new->getStateCount()-1),old_path_states[i]))
            {
                ...
                //--------------------------------------------------------
                path_index_DM.push_back(path_boost_DM[i]);
                //--------------------------------------------------------
                index = i;
                break;
            }
