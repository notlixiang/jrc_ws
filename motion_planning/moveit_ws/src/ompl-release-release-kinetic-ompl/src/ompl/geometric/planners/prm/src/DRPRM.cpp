...
std::vector<int> path_boost_DR;
std::vector<int> path_index_DR;

ompl::geometric::DRPRM::DRPRM(const base::SpaceInformationPtr &si) : base::Planner(si, "DRPRM")
{
//    specs_.approximateSolutions = true;
//    specs_.directed = true;

    std::cout<<"DRPRM"<<std::endl<<std::endl;

    std::ifstream ist("/home/smj/ros/jrc3/moveit_ws/graph_down_right.xml");
...
double ompl::geometric::DRPRM::m_distance(std::vector<double> q1, std::vector<double> q2)
{
    double distance_=0;
...
void ompl::geometric::DRPRM::adjust_graph()
{
...
ompl::geometric::DRPRM::~DRPRM()
{}

void ompl::geometric::DRPRM::clear()
{
    Planner::clear();
}

void ompl::geometric::DRPRM::setup()
{
    Planner::setup();
}

ompl::base::PlannerStatus ompl::geometric::DRPRM::solve(const base::PlannerTerminationCondition &ptc)
{
    checkValidity();
    adjust_graph();
    ...
    path_boost_DR.clear();
    for (; t != ipot_st; t = p[t])
        path_boost_DR.push_back(t);
    path_boost_DR.push_back(ipot_st);
    std::reverse(path_boost_DR.begin(), path_boost_DR.end());

    //--------------------------------------------------------
//    for(unsigned int i=0;i<path_boost_DR.size();i++)
//    {
//        std::cout<<path_boost_DR[i]<<"   ";
//    }
//    std::cout<<std::endl<<std::endl;
    ...
    for (unsigned int i = 0 ; i < path_boost_DR.size() ; i++)
    {
        ...
        for (unsigned int j = 0 ; j < si_->getStateDimension() ; ++j)
        {
            rstate->values[j] = vectormap[path_boost_DR[i]][j];
        }
        path->append(rstate);
    }
    ...
    //--------------------------------------------------------
//    boost::property_map<Graph_,vertex_state_str>::type stringmap=get(vertex_state_str(),g);
//    for(unsigned int m=0;m<path_index_DR.size();m++)
//    {
//        std::cout<<"path id "<<path_index_DR[m];
//        std::cout<<stringmap[path_index_DR[m]]<<std::endl;
//    }
...
void ompl::geometric::DRPRM::optimizationPath(PathGeometric *path_old,PathGeometric *path_new)
{
    ...
    //--------------------------------------------------------
    path_index_DR.clear();
    path_index_DR.push_back(path_boost_DR[0]);
    //--------------------------------------------------------
    ...
    while(index != (old_path_states.size()-1))
    {
        ...
        for(i=(old_path_states.size()-1);i>(index+1);i--)
        {
            if(si_->checkMotion(path_new->getState(path_new->getStateCount()-1),old_path_states[i]))
            {
                ...
                //--------------------------------------------------------
                path_index_DR.push_back(path_boost_DR[i]);
                //--------------------------------------------------------
                index = i;
                break;
            }
