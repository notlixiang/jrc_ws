#include "ompl/geometric/planners/prm/DESKMPRM.h"
#include "ompl/base/goals/GoalSampleableRegion.h"
#include "ompl/base/goals/GoalState.h"
#include "ompl/tools/config/SelfConfig.h"
#include <limits>

std::vector<int> path_boost_DESKM;
std::vector<int> path_index_DESKM;

ompl::geometric::DESKMPRM::DESKMPRM(const base::SpaceInformationPtr &si) : base::Planner(si, "DESKMPRM")
{
//    specs_.approximateSolutions = true;
//    specs_.directed = true;

    std::cout<<"DESKMPRM"<<std::endl<<std::endl;

    std::ifstream ist("/home/smj/ros/jrc3/moveit_ws/graph_desk_middle.xml");
...
double ompl::geometric::DESKMPRM::m_distance(std::vector<double> q1, std::vector<double> q2)
{
    double distance_=0;
...
void ompl::geometric::DESKMPRM::adjust_graph()
{
...
ompl::geometric::DESKMPRM::~DESKMPRM()
{}

void ompl::geometric::DESKMPRM::clear()
{
    Planner::clear();
}

void ompl::geometric::DESKMPRM::setup()
{
    Planner::setup();
}

ompl::base::PlannerStatus ompl::geometric::DESKMPRM::solve(const base::PlannerTerminationCondition &ptc)
{
    checkValidity();
    adjust_graph();
    ...
    int t = ipot_end;
    path_boost_DESKM.clear();
    for (; t != ipot_st; t = p[t])
        path_boost_DESKM.push_back(t);
    path_boost_DESKM.push_back(ipot_st);
    std::reverse(path_boost_DESKM.begin(), path_boost_DESKM.end());

    //--------------------------------------------------------
//    for(unsigned int i=0;i<path_boost_DESKM.size();i++)
//    {
//        std::cout<<path_boost_DESKM[i]<<"   ";
//    }
//    std::cout<<std::endl<<std::endl;
    ...
    for (unsigned int i = 0 ; i < path_boost_DESKM.size() ; i++)
    {
        ...
        for (unsigned int j = 0 ; j < si_->getStateDimension() ; ++j)
        {
            rstate->values[j] = vectormap[path_boost_DESKM[i]][j];
        }
        path->append(rstate);
    }
    ...
    //--------------------------------------------------------
//    boost::property_map<Graph_,vertex_state_str>::type stringmap=get(vertex_state_str(),g);
//    for(unsigned int m=0;m<path_index_DESKM.size();m++)
//    {
//        std::cout<<"path id "<<path_index_DESKM[m];
//        std::cout<<stringmap[path_index_DESKM[m]]<<std::endl;
//    }
...
void ompl::geometric::DESKMPRM::optimizationPath(PathGeometric *path_old,PathGeometric *path_new)
{
    ...
    //--------------------------------------------------------
    path_index_DESKM.clear();
    path_index_DESKM.push_back(path_boost_DESKM[0]);
    //--------------------------------------------------------
    ...
    while(index != (old_path_states.size()-1))
    {
        ...
        for(i=(old_path_states.size()-1);i>(index+1);i--)
        {
            if(si_->checkMotion(path_new->getState(path_new->getStateCount()-1),old_path_states[i]))
            {
                ...
                //--------------------------------------------------------
                path_index_DESKM.push_back(path_boost_DESKM[i]);
                //--------------------------------------------------------
                index = i;
                break;
            }
