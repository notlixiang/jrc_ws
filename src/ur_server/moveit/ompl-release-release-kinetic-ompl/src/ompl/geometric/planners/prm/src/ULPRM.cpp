...
std::vector<int> path_boost_UL;
std::vector<int> path_index_UL;

ompl::geometric::ULPRM::ULPRM(const base::SpaceInformationPtr &si) : base::Planner(si, "ULPRM")
{
//    specs_.approximateSolutions = true;
//    specs_.directed = true;

    std::cout<<"ULPRM"<<std::endl<<std::endl;

    std::ifstream ist("/home/smj/ros/jrc3/moveit_ws/graph_up_left.xml");
...
double ompl::geometric::ULPRM::m_distance(std::vector<double> q1, std::vector<double> q2)
{
    double distance_=0;
...
void ompl::geometric::ULPRM::adjust_graph()
{
...
ompl::geometric::ULPRM::~ULPRM()
{}

void ompl::geometric::ULPRM::clear()
{
    Planner::clear();
}

void ompl::geometric::ULPRM::setup()
{
    Planner::setup();
}

ompl::base::PlannerStatus ompl::geometric::ULPRM::solve(const base::PlannerTerminationCondition &ptc)
{
    checkValidity();
    adjust_graph();
    ...
    path_boost_UL.clear();
    for (; t != ipot_st; t = p[t])
        path_boost_UL.push_back(t);
    path_boost_UL.push_back(ipot_st);
    std::reverse(path_boost_UL.begin(), path_boost_UL.end());

    //--------------------------------------------------------
//    for(unsigned int i=0;i<path_boost_UL.size();i++)
//    {
//        std::cout<<path_boost_UL[i]<<"   ";
//    }
//    std::cout<<std::endl<<std::endl;
    ...
    for (unsigned int i = 0 ; i < path_boost_UL.size() ; i++)
    {
        ...
        for (unsigned int j = 0 ; j < si_->getStateDimension() ; ++j)
        {
            rstate->values[j] = vectormap[path_boost_UL[i]][j];
        }
        path->append(rstate);
    }
    ...
    //--------------------------------------------------------
//    boost::property_map<Graph_,vertex_state_str>::type stringmap=get(vertex_state_str(),g);
//    for(unsigned int m=0;m<path_index_UL.size();m++)
//    {
//        std::cout<<"path id "<<path_index_UL[m];
//        std::cout<<stringmap[path_index_UL[m]]<<std::endl;
//    }
...
void ompl::geometric::ULPRM::optimizationPath(PathGeometric *path_old,PathGeometric *path_new)
{
    ...
    //--------------------------------------------------------
    path_index_UL.clear();
    path_index_UL.push_back(path_boost_UL[0]);
    //--------------------------------------------------------
    ...
    while(index != (old_path_states.size()-1))
    {
        ...
        for(i=(old_path_states.size()-1);i>(index+1);i--)
        {
            if(si_->checkMotion(path_new->getState(path_new->getStateCount()-1),old_path_states[i]))
            {
                ...
                //--------------------------------------------------------
                path_index_UL.push_back(path_boost_UL[i]);
                //--------------------------------------------------------
                index = i;
                break;
            }
