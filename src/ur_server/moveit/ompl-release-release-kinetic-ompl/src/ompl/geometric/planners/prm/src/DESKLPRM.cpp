#include "ompl/geometric/planners/prm/DESKLPRM.h"
#include "ompl/base/goals/GoalSampleableRegion.h"
#include "ompl/base/goals/GoalState.h"
#include "ompl/tools/config/SelfConfig.h"
#include <limits>

std::vector<int> path_boost_DESKL;
std::vector<int> path_index_DESKL;

ompl::geometric::DESKLPRM::DESKLPRM(const base::SpaceInformationPtr &si) : base::Planner(si, "DESKLPRM")
{
//    specs_.approximateSolutions = true;
//    specs_.directed = true;

    std::cout<<"DESKLPRM"<<std::endl<<std::endl;

    std::ifstream ist("/home/smj/ros/jrc3/moveit_ws/graph_desk_left.xml");
...
double ompl::geometric::DESKLPRM::m_distance(std::vector<double> q1, std::vector<double> q2)
{
    double distance_=0;
...
void ompl::geometric::DESKLPRM::adjust_graph()
{
...
ompl::geometric::DESKLPRM::~DESKLPRM()
{}

void ompl::geometric::DESKLPRM::clear()
{
    Planner::clear();
}

void ompl::geometric::DESKLPRM::setup()
{
    Planner::setup();
}

ompl::base::PlannerStatus ompl::geometric::DESKLPRM::solve(const base::PlannerTerminationCondition &ptc)
{
    checkValidity();
    adjust_graph();
    ...
    int t = ipot_end;
    path_boost_DESKL.clear();
    for (; t != ipot_st; t = p[t])
        path_boost_DESKL.push_back(t);
    path_boost_DESKL.push_back(ipot_st);
    std::reverse(path_boost_DESKL.begin(), path_boost_DESKL.end());

    //--------------------------------------------------------
//    for(unsigned int i=0;i<path_boost_DESKL.size();i++)
//    {
//        std::cout<<path_boost_DESKL[i]<<"   ";
//    }
//    std::cout<<std::endl<<std::endl;
    ...
    for (unsigned int i = 0 ; i < path_boost_DESKL.size() ; i++)
    {
        ...
        for (unsigned int j = 0 ; j < si_->getStateDimension() ; ++j)
        {
            rstate->values[j] = vectormap[path_boost_DESKL[i]][j];
        }
        path->append(rstate);
    }
    ...
    //--------------------------------------------------------
//    boost::property_map<Graph_,vertex_state_str>::type stringmap=get(vertex_state_str(),g);
//    for(unsigned int m=0;m<path_index_DESKL.size();m++)
//    {
//        std::cout<<"path id "<<path_index_DESKL[m];
//        std::cout<<stringmap[path_index_DESKL[m]]<<std::endl;
//    }
...
void ompl::geometric::DESKLPRM::optimizationPath(PathGeometric *path_old,PathGeometric *path_new)
{
    ...
    //--------------------------------------------------------
    path_index_DESKL.clear();
    path_index_DESKL.push_back(path_boost_DESKL[0]);
    //--------------------------------------------------------
    ...
    while(index != (old_path_states.size()-1))
    {
        ...
        for(i=(old_path_states.size()-1);i>(index+1);i--)
        {
            if(si_->checkMotion(path_new->getState(path_new->getStateCount()-1),old_path_states[i]))
            {
                ...
                //--------------------------------------------------------
                path_index_DESKL.push_back(path_boost_DESKL[i]);
                //--------------------------------------------------------
                index = i;
                break;
            }
