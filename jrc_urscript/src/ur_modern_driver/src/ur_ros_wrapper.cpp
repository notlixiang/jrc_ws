...
#include "ur_msgs/Single_JointPosition.h"
#include "ur_msgs/Single_JointPositionRequest.h"
#include "ur_msgs/Single_JointPositionResponse.h"
#include "ur_msgs/trajectory.h"
#include "ur_msgs/Multi_JointPosition.h"
#include "ur_msgs/Multi_JointPositionRequest.h"
#include "ur_msgs/Multi_JointPositionResponse.h"
...
protected:
    ...
    ros::ServiceServer single_joint_position_srv_;
    ros::ServiceServer multi_joint_position_srv_;
    ros::ServiceServer movel_joint_position_srv_;
    ros::ServiceServer Multi_movel_joint_position_srv_;
...
public:
	RosWrapper(std::string host, int reverse_port) :
		...
		if (robot_.start()) {
            ...
            single_joint_position_srv_ = nh_.advertiseService("ur_driver/single_joint_position",
                    &RosWrapper::single_joint_position, this);
            multi_joint_position_srv_ = nh_.advertiseService("ur_driver/multi_joint_position",
                    &RosWrapper::multi_joint_position, this);
            movel_joint_position_srv_ = nh_.advertiseService("ur_driver/movel_joint_position",
                    &RosWrapper::movel_joint_position, this);
            Multi_movel_joint_position_srv_ = nh_.advertiseService("ur_driver/multi_movel_joint_position",
                    &RosWrapper::multi_movel_joint_position, this);
		}
	}
...
private:
    ...
    bool is_reached(std::vector<double> target,double max_time)
    {
        ...
        while(1)
        {
            ...
            double e0 = fabs(joint_msg.position[0] - target[0]);
            double e1 = fabs(joint_msg.position[1] - target[1]);
            double e2 = fabs(joint_msg.position[2] - target[2]);
            double e3 = fabs(joint_msg.position[3] - target[3]);
            double e4 = fabs(joint_msg.position[4] - target[4]);
            double e5 = fabs(joint_msg.position[5] - target[5]);
            if( e0<0.0005 && e1<0.0005 && e2<0.0005 && e3<0.0005 && e4<0.001 && e5<0.001)
            {
                return true;
            }

            auto t = std::chrono::high_resolution_clock::now();
            std::chrono::duration<double, std::milli> fp_ms = t - t0;
            if(fp_ms.count()>max_time)
            {
                for(int i=0;i<6;i++)
                    std::cout<<joint_msg.position[i]<<"    ";
                std::cout<<std::endl;
                for(int j=0;j<6;j++)
                    std::cout<<target[j]<<"     ";
                std::cout<<std::endl;
                ROS_ERROR("e1: %f,   e2: %f   ,   e3: %f   ,   e4: %f   ,   e5: %f   ,   e6: %f",e0,e1,e2,e3,e4,e5);
                ROS_ERROR("execute time out!");
                return false;
            }
        }
    }

    bool multi_joint_position(ur_msgs::Multi_JointPositionRequest& req,
                              ur_msgs::Multi_JointPositionResponse& resp){
        std_msgs::String msg;
        std::string head = "def process():\n";
        std::string str1 = " movej([";
        std::string str2 = ",";
        std::string str3 = "], a=1.4, v=1.05, t=";
        //std::string str3 = "], a=4, v=3, t=";
        std::string str4 = ", r=0.02) \n";
        std::string str4_ = ", r=0) \n";
        std::string tail = "end\n";
        ...
        {
            if(req.points[i].positions.size() != 6)
            {
                resp.result = false;
                return true;
            }

            std::string joint1_p_s,joint2_p_s,joint3_p_s,joint4_p_s,joint5_p_s,joint6_p_s;
            std::stringstream ss1,ss2,ss3,ss4,ss5,ss6;
            ss1<<req.points[i].positions[0];
            ss1>>joint1_p_s;
            ss2<<req.points[i].positions[1];
            ss2>>joint2_p_s;
            ss3<<req.points[i].positions[2];
            ss3>>joint3_p_s;
            ss4<<req.points[i].positions[3];
            ss4>>joint4_p_s;
            ss5<<req.points[i].positions[4];
            ss5>>joint5_p_s;
            ss6<<req.points[i].positions[5];
            ss6>>joint6_p_s;

            std::string m_time;
            std::stringstream m_time_ss;
            if((req.points[i].time<=3) && (req.points[i].time>=1) && (req.points[i].use_time == true))
            {
                m_time_ss<<req.points[i].time;
                m_time_ss>>m_time;
            }
            else
            {
            ...
            if(i == (req.points.size()-1))
            {
                msg.data += str1+joint1_p_s+str2+joint2_p_s+str2+joint3_p_s+str2+joint4_p_s+str2
                        +joint5_p_s+str2+joint6_p_s+str3+m_time+str4_;
            }
            else
            {
                msg.data += str1+joint1_p_s+str2+joint2_p_s+str2+joint3_p_s+str2+joint4_p_s+str2
        ...
        for(int i=0;i<6;i++)
            target.push_back(req.points[req.points.size()-1].positions[i]);

        bool execute_result = is_reached(target,4000.0*req.points.size());
        if(execute_result)
        {
            resp.result = true;
            return true;
    ...
    bool multi_movel_joint_position(ur_msgs::Multi_JointPositionRequest& req,
                                    ur_msgs::Multi_JointPositionResponse& resp)
    {
        std_msgs::String msg;
        std::string head = "def process():\n";
        std::string str1 = " movel([";
        std::string str2 = ",";
        std::string str3 = "], a=1.2, v=0.25, t=";
        //std::string str3 = "], a=2.4, v=0.5, t=";
        std::string str4 = ", r=0) \n";
        std::string tail = "end\n";
        ...
        for(int i=0;i<req.points.size();i++)
        {
            ...
            msg.data += str1+joint1_p_s+str2+joint2_p_s+str2+joint3_p_s+str2+joint4_p_s+str2
                    +joint5_p_s+str2+joint6_p_s+str3+m_time+str4;
        }
    ...
    bool single_joint_position(ur_msgs::Single_JointPositionRequest& req,
                               ur_msgs::Single_JointPositionResponse& resp){
        if(req.positions.size() != 6)
        {
            ROS_ERROR("The joint number should be 6!");
            resp.result = false;
            return true;
        }
        ...
        std::string m_time;
        std::stringstream m_time_ss;
        if(req.time<=3 && req.time>=1 && (req.use_time == true))
        {
            m_time_ss<<req.time;
            m_time_ss>>m_time;
        }
        ...
        std::vector<double> target;
        for(int i=0;i<6;i++)
            target.push_back(req.positions[i]);

        bool execute_result = is_reached(target,5000);
        if(execute_result)
        {
            resp.result = true;
            return true;
    ...
    bool movel_joint_position(ur_msgs::Single_JointPositionRequest& req,
                              ur_msgs::Single_JointPositionResponse& resp){
       if(req.positions.size() != 6)
       {
           ROS_ERROR("The joint number should be 6!");
