...
void PPF6DDetector::setParameters(const double distSampling, const double angleStep,
                  const double rotationThred, const double positionThred)
{
    dist_sampling = distSampling;
    angle_step = angleStep;
...
void PPF6DDetector::preProcessing(CloudPointT1Ptr pc, CloudPointT2 &pc_with_normals, CloudPointT1 &pc_show, bool training)
{
    if(training)
    {
...
//void PPF6DDetector::match(CloudPointT2Ptr pc, std::vector<obj6D::Pose6DPtr>& results, int modelPointCloudSize,const double relativeSceneSampleStep)
//{
...
////    cout<<"match init! pc_num:"<<numRefPoints<<endl;
//#pragma omp parallel for
//    for (int i = 0; i < numRefPoints; i += sceneSamplingStep)
//    {
...
//        uint* accumulator = (uint*)calloc(modelPointCloudSize*numAngles, sizeof(uint));
//        uint* flag = (uint*)calloc(angle_dim*angle_dim*angle_dim*dist_dim, sizeof(uint));
////        cout<<"fill hash start!"<<endl;
//        for (int j = 0; j < numRefPoints; j ++)
//        {
//            if (i!=j)
//            {
...
//                cv::Vec4d f = cv::Vec4d::all(0);
//                computePPFFeatures(p1, n1, p2, n2, f);
//                if(f[3]>dist_threshold){continue;}
//                int idx;
//                KeyType hashValue = hashPPF(f, angle_step*CV_PI/180.0f, distance_step, idx);
////                cout<<"hashValue!"<<endl;
...
////                cout<<"hashtableGetBucketHashed!"<<endl;
//                while (node)
//                {
////                     cout<<"test!"<<endl;
...
////                    cout<<"test4! index:"<<accIndex<<" corrI:"<<corrI<<" alpha_index:"<<alpha_index<<" total:"<<numRefPoints*numAngles<<endl;
////                    cout<<"accumulator[accIndex]:"<<accumulator[accIndex]<<endl;
    ...
    for (size_t i = 0; i < numRefPoints; i += sceneSamplingStep)
    {
        ...
        if(kdtree.radiusSearch(referencePoint,radius,pointIdxRadiusSearch,pointRadiusSquaredDistance)>0)
        {
            ...
            for(size_t j = 1; j < pointIdxRadiusSearch.size(); ++j)
            {
                ...
                // if(Distance(p1,p2)<3*distance_step)
                //    continue;
                if((TAngle3Normalized(n1,n2)/CV_PI*180.0f<20)&&(Distance(p1,p2)<6*distance_step))
                {
                    // cout<<"dis:"<<Distance(p1,p2)<<endl;
                    // cout<<"angle:"<<TAngle3Normalized(n1,n2)/CV_PI*180.0f<<endl;
                    continue;
                }
                cv::Vec3d p2t;
                double alpha_scene;
